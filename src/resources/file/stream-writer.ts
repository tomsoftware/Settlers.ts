...
export class StreamWriter {
		private log: LogHandler = new LogHandler('StreamWriter');
		...
		public getByte(offset: number): number {
			if ((offset < 0) || (offset > this.data.length)) {
				this.log.error('write out of data: size: ' + this.data.length + ' @ ' + offset);
				return -1;
			}
		...
		public setByte(value: number): boolean {
			if ((this.pos < 0) || (this.pos > this.data.length)) {
				this.log.error('write out of data: size: ' + this.data.length + ' @ ' + this.pos);
				return false;
			}
