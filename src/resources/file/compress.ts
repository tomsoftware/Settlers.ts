...
export class Compression {
	...
	///  </summary>
	SymbolAlphabet: number[] = new Array(Compression.SYMBOLS);

	//
	SymbIndexTable: number[] = [];
	...
	private constructor() {
		...
		this.SymbolCounter = new Array(Compression.SYMBOLS);
	}

	private buildInitialAlphabet() {
		const result = new Array(Compression.SYMBOLS);
		...
		for (let i = 1; (i <= 273); i++) {
			if (result.find((j) => j === i) < 0) {
				result.push(i);
			}
		}

		this.SymbolAlphabet = result;
		this.SymbIndexTable = [
	...
	public pack(src: Uint8Array, srcLen: number): BinaryReader {
		...
		while ((outPos < srcLen)) {
			...
			for (tabPos = 0; tabPos < 16; tabPos++) {
				...
				refLen = (1 << this.SymbIndexTable[tabPos << 1]) - 1;
				// this val is added to the offset
				refOff = this.SymbIndexTable[(tabPos << 1) + 1];
			...
			bitwtr.write(remain, this.SymbIndexTable[tabPos << 1]);
			...
			if (symbol === 0x110) {
				...
				for (let i = 0; i < 32; i += 2) {
					back = this.SymbIndexTable[i];
					if (i >= 2) back -= this.SymbIndexTable[i - 2];
					while (back > 0) {
						back--;
						bitwtr.write(0, 1);
					}
