module Settlers {
    ...
    /**
	 * 	ZLib like compressing
	*/
	export class Uncompress {

		private log: LogHandler = new LogHandler("Uncompress");
		...
		private createCodeTableFromQuantities(codes: Uint16Array, quantities: Uint32Array) {
			let tmpQuantity = new Uint32Array(274);

			for (let i = 0; i < 274; i++) {
				let count = quantities[i];

				//- create a snapshot of the quantitys
				tmpQuantity[i] = count;
				codes[i] = i;

				//- we reduct the original quantity by 2 to the impact for the next CreateCodeTableFromFrequency() call
		...
		private sortCodeTable(codes: Uint16Array, quantitys: Uint32Array, lenght: number) {
			let pos = 0;
			let delta = 0;
			let srcCode = 0;
			let srcQuantity = 0;
			...
			do {
				for (let i = stepSize; i < lenght; i++) {
					...
					while (pos >= stepSize) {
						...
						delta = quantitys[srcPos] - srcQuantity;
						if (delta == 0) {
						...
						quantitys[pos] = quantitys[srcPos];
