    ...
    export class MapFile {
        ...
        private readFileSections(data: BinaryReader, offset:number) {
            ...
            while(offset > 0) {
                ...
                this.log.debug(newSection.toString());

                if (newSection.sectionType == MapSectionType.CommentText) {
                    let sectionReader:BinaryReader = newSection.getReader();
                    this.log.debug(sectionReader.readString());
                }
                else {
